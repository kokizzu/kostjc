...
var viewList = map[string]string{
	...
	`UserMissingDataReport`: `../svelte/userMissingDataReport.html`, // ../svelte/userMissingDataReport.svelte
	`UserOccupancyReport`:   `../svelte/userOccupancyReport.html`,   // ../svelte/userOccupancyReport.svelte
}
...
func (v *Views) RenderUserMissingDataReport(c *fiber.Ctx, m M.SX) error {
	c.Set("Content-Type", "text/html; charset=utf-8")
	return c.SendString(v.cache[`UserMissingDataReport`].Str(m))
}

func (v *Views) RenderUserOccupancyReport(c *fiber.Ctx, m M.SX) error {
	c.Set("Content-Type", "text/html; charset=utf-8")
	return c.SendString(v.cache[`UserOccupancyReport`].Str(m))
}
