...
const [watch, serve, minify, debug, logVars, ssr] = ['--watch', '--serve', '--minify', '--debug', '--log-vars', '--ssr'].map(s =>
  process.argv.includes(s),
);
const debug_console_log = (args, returnIndex = 0) => (debug && console.log(...args), args[returnIndex]);
const shouldKeepSvelteWarning = warning => {
  const filename = warning?.filename || '';

  if (filename.includes('/node_modules/.pnpm/svelte-select@') || filename.includes('/node_modules/svelte-select/')) {
    return false;
  }
  ...
  const buildOptions = {
    ...
    plugins: [
      ...
      sveltePlugin({
        ...
        filterWarnings: shouldKeepSvelteWarning,
      }),
    ],
    sourcemap: false,
    minify,
...
(async () => {
  ...
  async function saveFiles(files = builder, layoutChanged = false) {
    ...
    for (const [path, data] of Object.entries(output)) {
      ...
      if (ssr) {
        const ssrPath = resolve('.ssr_temp', relative(__dirname, path) + '.cjs');
