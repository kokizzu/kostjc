...
//go:generate gomodifytags -all -add-tags json,form,query,long,msg -transform camelcase --skip-unexported -w -file StaffBooking.go
//go:generate replacer -afterprefix "Id\" form" "Id,string\" form" type StaffBooking.go
//go:generate replacer -afterprefix "json:\"id\"" "json:\"id,string\"" type StaffBooking.go
//go:generate replacer -afterprefix "By\" form" "By,string\" form" type StaffBooking.go
//go:generate farify doublequote --file StaffBooking.go

type (
	StaffBookingIn struct {
		RequestCommon
...
const (
	StaffBookingAction = `staff/booking`

	ErrStaffBookingNotFound      = `booking not found`
	ErrStaffBookingSaveFailed    = `failed to save booking`
	ErrStaffBookingDeleteFailed  = `failed to delete booking`
	ErrStaffBookingRestoreFailed = `failed to restore booking`
)

var StaffBookingMeta = AdminBookingMeta

func (d *Domain) StaffBooking(in *StaffBookingIn) (out StaffBookingOut) {
	...
	case zCrud.CmdUpsert, zCrud.CmdDelete, zCrud.CmdRestore:
		...
		if bk.Id > 0 {
			if !bk.FindById() {
				out.SetError(400, ErrStaffBookingNotFound)
				return
			}
		...
		if len(in.Facilities) > 0 {
			...
			if err != nil {
				out.SetError(500, ErrStaffBookingSaveFailed)
				return
			} else {
