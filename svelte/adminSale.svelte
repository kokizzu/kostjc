<script>
  ...
  async function OnEdit(/** @type {any} */ id, /** @type {any[]} */ payloads) {
    ...
    changeIDR: Number(payloads[10]),
    debtIDR: Number(payloads[11]),
    topupIDR: Number(payloads[12]),
    donation: Number(payloads[13]),
    totalPriceIDR: Number(payloads[14]),
    salesDate: String(payloads[15]),
    paidAt: String(payloads[16]),
    note: String(payloads[17]),
  }
    const i = /** @type {any}*/ ({
      pager,
      sale,
